...
## 2026-09-21（週次・定期実行・隔週大幅更新）

- 実行日時: 2026-09-21 09:07 JST（Asia/Tokyo。無人の定期実行）
- ブランチ: claude/seo-weekly-2026-09-21（新規ドラフトPR運用）
- 事前確認: git status（クリーン）／origin/main最新化（69aa145）／CLAUDE.md（リポジトリに存在せず）／docs/seo配下の管理ファイル一式／既存ブランチ・オープンPRを確認
  - オープンPR #43（`codex/draft-mimoto-hosho-keiyaku-checklist`、2026-09-19作成、ドラフト）: 身元保証サービスの契約前チェックリスト下書きで、本セッションの対象（article11の大幅更新）とは扱う記事・範囲とも重複しないことを確認。重複作業なし
  - `claude/seo-weekly-*` の自動運用ブランチはリモートに残っておらず（過去分はマージ・クローズ済み）、新規ブランチ名の衝突なし
- 判定: 編集カレンダーの次回未着手タスクは2026-09-28〜10-02（article06のデジタル終活チェックリスト化）で今週は対象外。サイト全体の障害・リンク切れ・誤ったnoindex・ビルド失敗（優先順位1）、重大な誤解を招くYMYL情報の放置（優先順位2）はいずれも確認されず。前回大幅更新（2026-08-10 article05）から本日で42日経過し、隔週の大幅更新基準（14日）を大幅に超過していたため、本プロンプト5章の優先順位1位「終活チェックリスト」（article11）の大幅更新に今回着手した（2026-08-24・2026-09-07の両実行で「次回最優先」として見送られていた分）
- 実施した作業（`articles/article11.html` を大幅改修）:
  1. **一次情報の追加**: STEP1「借金・ローン・保証の有無」の項目に、article15で確認済み・司法書士/税理士レビュー待ちとして登録済みの一次情報（裁判所「相続の放棄の申述」、相続放棄・限定承認は原則3か月以内）を同一内容のまま転記し、記事内から直接一次情報URLへリンクできるようにした
  2. **会話例の新設（必須要素の充足）**: 「家族と共有する方法」章に会話例3パターン（書き始めを伝えるとき／年1回の見直しを誘うとき／親に自分のチェックリストを見せて誘うとき）を追加。実在の相談事例・体験談としてではなく、一般的な言い回しの例として記載（架空の実体験・相談事例の禁止に抵触しないよう、Q&A形式の会話例に留めた）
  3. **内部リンクの是正**: article11は柱記事だが、公開時期の違いにより/parent-shukatsu/siblings/・/parent-shukatsu/living-alone/・/cost/・/where-to-ask/（いずれも2026-08〜09公開、article11からは未リンク。4記事側は既にarticle11へリンク済みで片方向だった）への内部リンクが欠けていたため、状況別セクション（親の終活を進めたい方→siblings・living-alone／おひとりさまの方→where-to-ask）・専門家へ相談すべきケース（→where-to-ask）・関連記事欄（4記事すべて）へ追加し相互リンク化した
  4. **FAQの追加**: Q5「何をどこに相談すればいいか分からない場合はどうすればいいですか？」を新設（4問→5問）。/where-to-ask/への導線と、本記事内の専門家相談セクションへの参照を回答に含めた。FAQPage JSON-LDも同期
  5. `dateModified`・記事内の最終更新日表示を2026-09-21へ更新（本文構成を伴う実質更新のため）。sitemap.xmlのarticle11 lastmodも同期
- 変更したURL: `https://shukatsu-guide.jp/articles/article11.html`（本文複数箇所・FAQ・構造化データ）。他ページの本文変更なし
- 検索意図: 変更なし（「終活 チェックリスト」「終活 何から」等の既存クエリのまま。新規セクション・内部リンクは既存の検索意図を補強する追加）
- 内部リンク: 上記「実施した作業」3参照。`internal-link-map.csv`に新規行を追加（article11→siblings/living-alone/cost/where-to-ask）
- 技術変更: FAQPage JSON-LDに質問1件を追加（DOM↔JSON-LD一致は`node tools/check-site.js`で確認）。BreadcrumbList・Article JSON-LDの構造変更なし
- PDF、DOCX変更: なし（`downloads/shukatsu-checklist.pdf`は既存のまま。本文の項目数・番号は変更していないため、PDFとの整合に影響なし）
- 参考情報: 新規の法制度・統計・固有名詞の追加なし。article15で既に確認済み・レビュー待ち登録済みの一次情報（裁判所「相続の放棄の申述」）を同一内容のまま転記したのみ。転記に伴い`source-policy.md`の該当行に article11 を追記し、`ymyl-review-queue.md`にもarticle15と同様の未レビュー事項として新規登録した
- テスト結果: `node tools/check-site.js` → **ALL CHECKS PASSED**（HTML files: 39、internal links checked: 1212）／`node tools/check-click-tracking.js` → **PASSED**（public HTML: 35、affiliate links: 34、CTAs: 13、products: 12）／`git diff --check` → PASS。Playwright（このセッションではグローバルインストール済みのパッケージをNODE_PATH経由で参照。ローカルnode_modulesには存在せず）でmodule解決を確認したうえで、article11.htmlをモバイル(390px)・デスクトップ(1280px)でDOM計測（scrollWidth<=clientWidth）し横スクロールなしを確認。FAQ（5問に増加）のDOM開閉動作もPlaywrightで確認。孤立ページの機械的全件走査（tools/pdf/*.html・404.htmlを除外）で新たな孤立ページがないことを確認。downloads/配下5ファイルの実在、robots.txt・sitemap.xmlの内容も個別確認
- Search Console分析: `docs/seo/sc-data/`が引き続き未配置のためデータなし。表示回数・CTR・掲載順位・前月比較は「データ不足」として記録し、数値の推測は行っていない
- 季節需要（6〜10週間後＝2026-11-02頃〜11-30頃）確認: 該当する時期の年末年始・帰省は編集カレンダー2026-11後半・12月前半に既に計画済みで、この時間軸に新規性の高い季節記事の追加着手は今回不要と判断
- 編集カレンダー確認: 次回未着手タスクは2026-09-28〜10-02（article06のデジタル終活チェックリスト化、大幅更新）。過去の未完了タスクの新たな遅延なし
- YMYLレビュー待ち: `ymyl-review-queue.md`にarticle11の相続放棄・限定承認の期限記述（article15と同一の一次情報URLを転記）を新規追加（既存項目は継続）
- 阻害要因: なし
- 次回予定: 編集カレンダー2026-09-28〜10-02週（article06のデジタル終活チェックリスト化）に着手予定。あわせて2026年10月が四半期ごとのエンディングノート比較記事（article08）の商品確認時期にあたるため、次回以降のいずれかの実行で対応を検討する
- PR: ドラフトPRを作成
...
- 本プロンプト14章「既存の作業中PRと同じ内容を扱わない」に従い、重複するブランチ・PRは作成せず、新規記事・大幅更新・内部リンク変更は行っていない
- 再検証として、PR #36のブランチ上で以下を再実行し、いずれも初回実施時と同じ結果で通過を確認した
  - `node tools/check-site.js` → ALL CHECKS PASSED（HTML files: 37、internal links checked: 1115）
  - `node tools/check-click-tracking.js` → PASSED（public HTML: 33、affiliate links: 34、CTAs: 13、products: 12）
  - `git diff --check` → PASS
- あわせてmain上でも `node tools/check-site.js` を実行し、PR #36マージ前の本番相当コード（HTML 36件・内部リンク1080件）が健全であることを確認した
- 季節需要（6〜10週間後 = 2026-10-12〜2026-11-09）を確認したが、該当する季節イベントなし（敬老の日・秋のお彼岸は既に終了、年末年始は10週間超先）。追加対応なし
- 編集カレンダーの次回未着手タスクは2026-09-07〜11週「終活の相談先比較」（未到来）。過去の遅延タスクなし
- Search Consoleデータ: `docs/seo/sc-data/` にCSV未配置のため引き続きデータ不足。数値の推測は行っていない
- 判定: 今回変更すべきものなし（PR #36が既に本タスクを充足しているため）。管理ファイルへのこの再検証記録のみを本ブランチへ追記
- ブランチ: `claude/seo-weekly-2026-08-31`（PR #36を更新。新規ブランチ・新規PRは作成せず）
- 次回予定: 2026-09-07〜11週「終活の相談先比較」。隔週の大幅更新候補article11は、PR #36のレビュー・公開判断後に優先して検討する
...
- ただしマージ直後（07:16:26Z）に自動レビューからP2指摘が1件入った。**PR #33のときと同じく、マージ後に届いたためそのPRでは対応できない**
- **指摘内容（妥当）**: `keyword-map.csv` では living-alone の最終実質更新日を 2026-08-25 に更新したのに、`parent-shukatsu/living-alone/index.html` の `dateModified`・`sitemap.xml` の lastmod・`content-inventory.csv` は 2026-08-24 のままで、**更新日メタデータが4か所で食い違っていた**。法的記述の訂正という実質的な内容更新を行ったのに、クローラーと他の管理台帳には旧版の日付を伝え続ける状態だった
- 対応（本PR）:
  - `living-alone`: `dateModified` 2026-08-24→2026-08-25、表示を「公開日：2026年8月24日／最終更新日：2026年8月25日」へ、sitemap lastmod と content-inventory の最終実質更新日も 2026-08-25 へ。あわせて content-inventory の状態が「公開(PR中)」のままだったため「公開」へ是正
  - `questions`: 同じ法的記述の訂正を行ったにもかかわらず日付が2026-07-11のままだったため、`dateModified`・表示・sitemap・content-inventory・keyword-map をすべて 2026-08-25 へ統一
  - いずれも**本文を伴う実質更新に対する日付更新**であり、「本文を変えず更新日だけを変える」行為には該当しない
- **運用上の反省（2件連続で同じ事象）**: PR #33・#34 ともに、レビュー指摘がマージ直後に届き、そのPRでは対応できなかった。ドラフトPRを作成してから自動レビューが走るまでに数分のラグがあるため、**マージ前にレビューコメントの有無を確認する余地があるかどうか、運営者と運用を相談したい**（自動運用側からマージ時期は制御できないため）
- テスト結果: 後述（コミット時に記載）

## 2026-08-25（PR #33マージ後の取りこぼし対応・別PRで再提出）

- **経緯（重要）**: PR #33 は 2026-08-25T03:54:02Z に head `38b5b02` の状態でマージされた（main=`7eb1911`）。しかしその直後に本セッションが同ブランチへ push した2コミット（`3ea4c03` 記録復元／`a2ef80c` レビュー指摘対応）は**マージに含まれず、mainへ反映されないまま PR #33 がクローズされた**
- **その結果、次の状態が本番に残っていた**:
  - `/parent-shukatsu/living-alone/`（3か所）と `/parent-shukatsu/questions/`（1か所）に、**無出典の断定的な法的記述**「財産管理の制度（任意後見など）は本人に判断能力があるうちにしか契約できません」が公開されたまま
  - `keyword-map.csv` の living-alone のYMYL区分が「低・レビュー不要」のまま（認知機能低下・介護・後見制度に言及するページとして不適切）
  - `ymyl-review-queue.md` に living-alone の登録がないまま
  - `automation-state.json` の completed_tasks から 2026-08-24 週次分の記録が欠落したまま
- **反省点**: PR #33 のレビュースレッドへ「修正しました（a2ef80c）」と返信したが、その時点で**PRは既にマージ済みであり、当該コミットは本番に反映されていなかった**。ブランチへのpush＝反映と即断せず、マージ状態を確認したうえで返信すべきだった
- 対応: マージ済みPRへ追加コミットすることはできないため、**最新mainから新ブランチ `claude/seo-followup-2026-08-25` を作成し、失われた2コミットをcherry-pickして別PRとして再提出**した（PR #33 の再オープンや同一変更の重複PR作成はしていない）
- 変更内容は下記「2026-08-25（PR #33のレビュー指摘対応）」および「PR #32マージ・PR #33統合の記録」の各セクションのとおりで、内容自体に変更はない
- テスト結果: `node tools/check-site.js` → ALL CHECKS PASSED（HTML 36件・内部リンク1080件）／`node tools/check-click-tracking.js` → PASSED（affiliate 34・CTA 13・products 12）。サイト内に当該断定表現が残っていないことも全文検索で確認
- 未解決事項: 本PRのマージは本番公開に直結するため運営者のご判断待ち。**未マージの間は上記の法的断定が本番に残る**点にご留意ください
- 次回予定: 2026-08-31〜09-04週（終活にかかる費用）。週次ルーチンは 2026-08-31T00:05Z に発火予定

## 2026-08-25（PR #33のレビュー指摘対応）

- PR #33 に自動レビュー（chatgpt-codex-connector）からP2指摘が2件。うち1件を対応し、1件は運営者の判断を仰ぐため保留
- **対応した指摘（P2: 無出典の法的断定）**: `/parent-shukatsu/living-alone/` の「財産管理の制度（任意後見など）は**本人に判断能力があるうちにしか契約できません**」という記述が、一次情報の裏付けなしに法制度の絶対的ルールを断定しており、かつkeyword-map.csv上は「低YMYL・レビュー不要」に分類されていたため、source-policy／ymyl-review-queueの管理を素通りしていた
  - 本文（sec8）・FAQ Q5・FAQPage JSON-LD の3か所を「任意後見などの財産管理の制度は、利用できる時期や条件が定められています。気になる変化があれば先延ばしにせず、利用の可否や手続きを公的窓口・専門家にご確認ください」へ修正。**読者にとって実務上重要な「先延ばしにしない」という要点は保持しつつ、当サイトが法的ルールを断定する形をやめた**
  - keyword-map.csv のYMYL区分を「低・不要」→「**中（認知機能低下・介護・後見制度に言及）・将来推奨**」へ是正
  - ymyl-review-queue.md に本ページの行を新規追加（司法書士／介護実務者によるレビュー対象として登録）
- **スコープ外だが同一の欠陥を既存の公開ページでも修正**: `/parent-shukatsu/questions/`（2026-07-11公開・main反映済み）の sec10 に**まったく同じ断定表現**が残っていたため、あわせて同じ文言へ修正した。PR #33（2026-08-24週タスク）の本来のスコープ外だが、同一の欠陥を新規ページだけ直して公開済みページに残すのは不適切と判断。**この1行のみスコープ外のため、不要であれば本コミットから外していただいて差し支えない**
- **保留した指摘（P2: 「執筆・監修」表記）**: 「認知機能低下・介護・後見制度に触れるページで `執筆・監修：終活カウンセラー1級` と表示するのは、医療・法務部分について専門家の監修を受けたと誤認させる」という指摘。**この表記はサイト全ページ共通の定型であり、`tools/check-site.js` も `articles/` 配下では必須項目として検査している**。1ページだけ変更すると表記が不統一になり、サイト全体で変更するかどうかは運営者ご自身の資格表示に関わる判断のため、自動運用側では変更せず運営者へ確認することとした
- テスト結果: `node tools/check-site.js` → ALL CHECKS PASSED（HTML 36件・内部リンク1080件）／`node tools/check-click-tracking.js` → PASSED（affiliate 34・CTA 13・products 12）。FAQのDOM↔JSON-LD一致もチェッカーで確認済み

## 2026-08-25（PR #32マージ・PR #33統合の記録）

- 運営者がPR #32（2026-08-17週＝兄弟姉妹の役割分担）をmainへスカッシュマージ（main=`fd82784`）。PR #33（2026-08-24週）はmain起点で分岐していたため8ファイルが衝突したが、**運営者自身がmainを取り込んで解消済み**（`38b5b02`）。新着記事は公開日順の最新3本（living-alone 8/24／siblings 8/17／obon 8/13）へ再構成され、PR #32側のドリフト是正も保持されていることを確認
- 本セッションでの追加対応: 上記マージの際に`automation-state.json`の`completed_tasks`から**2026-08-24分の記録が欠落**していたため復元した（`run-log.md`側には残っていた）
- **運営者がPR #32のマージ前にsiblings記事へ加えた修正（今後の執筆方針として重要）**:
  1. **親の自己決定を起点にする** — 「進め方の大原則」1番目を「きょうだい間で足並みをそろえる」から「**親本人の希望と、話してよい範囲を最初に確かめる**」へ差し替え
  2. **断定的な効果表現を避ける** — 「揉めごとの多くは防げます」→「記録だけで行き違いをなくせるとは限りません」、「トラブルをほぼ防げます」→「後から確認しやすくなります」
  3. **運営者の相談経験に帰属する記述を書かない** — 「終活カウンセラーとして相談を受けてきた経験からまとめました」「運営者が相談を受けてきた中でも、費用トラブルの多くは…」を削除し「一般的な情報としてまとめます」へ。**これは自動運用側が書くべきでなかった記述であり、本プロンプト12章の「架空の実体験・架空の相談事例」の禁止に抵触しうる。今後、運営者の実体験・相談実績に帰属させる表現は、運営者から明示的に提供された事実の範囲でのみ使用する**
  4. **個人情報の取り扱い** — 暗証番号・パスワード・本人確認情報をグループチャットや共有表に残さない。情報の種類ごとに共有範囲・保管場所・更新の伝え方を決める（`#info-management`）。親の同意なく配偶者等へ共有を広げない
  5. **代理権の限界** — 家族内で担当を決めても医療同意・金融手続きの代理権は生じない旨を、大原則直後の注意box・失敗パターン・専門家相談の境界線の3か所へ明記
- **検証手順の是正**: `tools/check-click-tracking.js`（アフィリエイトリンク数・CTA数・商品数の回帰チェック）の存在を本セッションは見落としていた。運営者が期待値を30→34本へ更新済み。**以後、`tools/check-site.js`とあわせて必ず両方を実行する**
- テスト結果: `node tools/check-site.js` → ALL CHECKS PASSED（HTML 36件・内部リンク1080件）／`node tools/check-click-tracking.js` → PASSED（affiliate 34・CTA 13・products 12）
- 未解決事項: PR #33のmainへのマージは本番公開に直結するため運営者のご判断待ち
- 次回予定: 2026-08-31〜09-04週（終活にかかる費用）。あわせて隔週の大幅更新としてarticle11（終活チェックリスト完全版）の改修を最優先で検討

## 2026-08-24（週次・定期実行）

- 前週のPR #32（兄弟姉妹での役割分担）は公開済みとして、最新mainへ統合してから作業した。
- `/parent-shukatsu/living-alone/` を新設。合鍵・見守り・孤立防止の確認、時期別チェックリスト、会話例、印刷用の記入シート、状況別の考え方、FAQを追加した。
- トップの新着記事は公開日の新しい順に3件（2026-08-24／2026-08-17／2026-08-13）へ更新。親の終活ハブ・sitemap・content-inventory・keyword-map・internal-link-map・editorial-calendarも両記事を残した状態で更新した。
- 医療・法律・金融の個別判断は扱わず、必要に応じて各窓口や専門家への相談を案内した。
- 統合状態でリンク、ページ内アンカー、JSON-LD、FAQ、sitemap、クリック計測を再確認した。
...
## 2026-09-14 PR #42 統合確認

- PR #40 → #41 → #42の順に統合。上記週次ログの「孤立中」「PR #40・#41未マージ」は作業時点の履歴で、今回の統合後の状態ではない。
